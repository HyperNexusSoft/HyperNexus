...
export function registerAgentCommand(program: Command): void {
  ...
  agent
    ...
    .action(async (opts, cmd) => {
      ...
      let squads: any[] = [];
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/squad.list', { signal: AbortSignal.timeout(3000) });
        if (res.ok) {
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        const [dRes, sRes] = await Promise.all([
          fetch('http://127.0.0.1:7779/trpc/director.status', { signal: AbortSignal.timeout(3000) }),
          fetch('http://127.0.0.1:7779/trpc/supervisor.status', { signal: AbortSignal.timeout(3000) }),
        ]);
