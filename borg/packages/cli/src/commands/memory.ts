...
Examples:
  ...
  $ tormentnexus memory add "Deploy with: pnpm build && pnpm start" -t procedural
    ...
    .action(async (content, opts) => {
      ...
      let persisted = false;
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/memory.saveContext', {
          method: 'POST',
  ...
  mem
    ...
    .action(async (query, opts, cmd) => {
      ...
      try {
        ...
        const res = await fetch(`http://127.0.0.1:7779/trpc/memory.semanticSearch?input=${input}`, { signal: AbortSignal.timeout(5000) });
        if (res.ok) {
  ...
  mem
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const res = await fetch(`http://127.0.0.1:7779/trpc/memory.getRecentObservations?input=${input}`, { signal: AbortSignal.timeout(3000) });
        if (res.ok) {
          const json = await res.json();
  ...
  mem
    ...
    .action(async (opts, cmd) => {
      ...
      let stats: any = {};
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/memory.getAgentStats', { signal: AbortSignal.timeout(3000) });
        if (res.ok) {
          const json = await res.json();
