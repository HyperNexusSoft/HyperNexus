...
export function registerMcpCommand(program: Command): void {
  ...
  mcp
    ...
    .action(async (opts, cmd) => {
      ...
      let servers: any[] = [];
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.listServers', { signal: AbortSignal.timeout(3000) });
        if (res.ok) {
  ...
  mcp
    ...
    .action(async (name) => {
      ...
      // Try tRPC first
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.connectServer', {
          method: 'POST',
      ...
      if (!started) {
        try {
          const listRes = await fetch('http://127.0.0.1:7779/trpc/mcp.listServers', { signal: AbortSignal.timeout(5000) });
          if (listRes.ok) {
  ...
  mcp
    ...
    .action(async (name) => {
      ...
      // Try tRPC first
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.disconnectServer', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ json: { name } }),
...
Examples:
  ...
  $ tormentnexus mcp add remote-api http://localhost:8080/mcp -t streamable-http
    ...
    .action(async (name, command, opts) => {
      const chalk = (await import('chalk')).default;
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcpServers.create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  mcp
    ...
    .action(async (name, opts) => {
      const chalk = (await import('chalk')).default;
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcpServers.delete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ json: { name } }),
          signal: AbortSignal.timeout(5000),
  ...
  mcp
    ...
    .action(async (name) => {
      ...
      try {
        ...
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.listServers', {
          signal: AbortSignal.timeout(5000),
        });
        if (res.ok) {
          const json = await res.json();
          ...
          try {
            const toolsRes = await fetch('http://127.0.0.1:7779/trpc/mcp.listTools', { signal: AbortSignal.timeout(5000) });
            if (toolsRes.ok) {
  ...
  mcp
    ...
    .action(async (opts) => {
      ...
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.traffic', {
          signal: AbortSignal.timeout(5000),
        });
        if (res.ok) {
          const json = await res.json();
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        if (opts.search) {
          ...
          const res = await fetch(`http://127.0.0.1:7779/trpc/mcp.searchTools?input=${input}`, { signal: AbortSignal.timeout(5000) });
          if (res.ok) {
            const json = await res.json();
          ...
          const res = await fetch('http://127.0.0.1:7779/trpc/mcp.listTools', { signal: AbortSignal.timeout(5000) });
          if (res.ok) {
            const json = await res.json();
  ...
  mcp
    ...
    .action(async (opts) => {
      ...
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/settings.get', { signal: AbortSignal.timeout(3000) });
        if (res.ok) {
          const json = await res.json();
...
Examples:
  ...
  $ tormentnexus mcp install --github anthropics/mcp-servers
    ...
    .action(async (pkg, opts) => {
      ...
      try {
        ...
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.addServer', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ json: {
            name: serverName,
  ...
  mcp
    ...
    .action(async (query, opts, cmd) => {
      ...
      let servers: any[] = [];
      try {
        const res = await fetch('http://127.0.0.1:7779/trpc/mcp.listServers', { signal: AbortSignal.timeout(5000) });
        if (res.ok) {
          const json = await res.json();
          servers = json?.result?.data ?? [];
        }
...
Supported clients:
  ...
  mcp
    ...
    .action(async (opts) => {
      ...
      try {
        // Get list of servers
        const listRes = await fetch('http://127.0.0.1:7779/trpc/mcp.listServers', {
          signal: AbortSignal.timeout(5000),
        });
        if (!listRes.ok) {
