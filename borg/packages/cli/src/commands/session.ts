...
export function registerSessionCommand(program: Command): void {
	...
	session
		...
		.action(async (opts, cmd) => {
			...
			let sessions: any[] = [];
			try {
				const res = await fetch("http://127.0.0.1:7779/trpc/session.list", {
  ...
  $ tormentnexus session start ./my-app --supervisor --auto-restart
    `,
		...
		.action(async (workdir, opts) => {
			...
			try {
				...
				const res = await fetch('http://127.0.0.1:7779/trpc/session.create', {
					method: 'POST',
	...
	session
		...
		.action(async (id) => {
			const chalk = (await import("chalk")).default;
			try {
				const res = await fetch('http://127.0.0.1:7779/trpc/session.stop', {
	...
	session
		...
		.action(async (id) => {
			const chalk = (await import("chalk")).default;
			try {
				const res = await fetch('http://127.0.0.1:7779/trpc/session.start', {
					method: 'POST', headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ json: { id } }),
					signal: AbortSignal.timeout(5000),
				});
