  ...
  program
    ...
    .action(async (opts) => {
      ...
      if (opts.json) {
        ...
        try {
          const res = await fetch('http://127.0.0.1:7779/health', { signal: AbortSignal.timeout(3000) });
          if (res.ok) info.server = await res.json();
        } catch { info.server = { status: 'stopped' }; }

        try {
          const res = await fetch('http://127.0.0.1:7779/trpc/mcp.getStatus', { signal: AbortSignal.timeout(3000) });
      ...
      const [health, mcpStatus, goHealth, catalogStats] = await Promise.all([
        fetch('http://127.0.0.1:7779/health', { signal: AbortSignal.timeout(3000) }).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch('http://127.0.0.1:7779/trpc/mcp.getStatus', { signal: AbortSignal.timeout(3000) }).then(r => r.ok ? r.json() : null).catch(() => null),
      ...
      // Cloud providers
      try {
        const cloudRes = await fetch('http://127.0.0.1:7779/trpc/cloudDev.listProviders', { signal: AbortSignal.timeout(2000) });
        if (cloudRes.ok) {
      ...
      // Harnesses
      try {
        const hRes = await fetch('http://127.0.0.1:7779/trpc/tools.detectCliHarnesses', { signal: AbortSignal.timeout(2000) });
        if (hRes.ok) {
