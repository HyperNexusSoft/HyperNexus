...
def fetch(url, timeout=25):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "TormentNexus/1.0", "Accept": "application/json, text/html, */*"})
...
def insert_batch(db, servers, source, existing):
    ...
    for s in servers:
        ...
        existing.add(url)
        try:
            db.execute("""INSERT OR IGNORE INTO links_backlog 
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (str(uuid.uuid4()), url, url, s.get("name","")[:200], s.get("description","")[:500],
                 json.dumps(["mcp",source]), source, False, "", "pending", 200, now, now, now))
            inserted += 1
        except Exception:
            pass
    return inserted
...
def scrape_awesome_mcp(db, existing):
    ...
    raw = fetch("https://raw.githubusercontent.com/punkpeye/awesome-mcp-servers/main/README.md")
    if not raw: return 0
    servers = []
    for name, url in re.findall(r'\[([^\]]+)\]\((https://github\.com/[^)]+)\)', raw):
        url = url.rstrip("/")
        if "awesome-mcp" in url.lower() or "discord" in url.lower(): continue
...
def scrape_github_search(db, existing):
    ...
    queries = [
        "mcp+server+topic:mcp", "mcp+server+language:python", "mcp+server+language:typescript",
        "mcp+server+language:go", "mcp+server+language:rust", "mcp+server+language:java",
        "model-context-protocol", "mcp-server+in:name", "mcp+tools+in:description",
        "mcp+database+server", "mcp+filesystem+server", "mcp+browser+server",
        "mcp+api+server", "mcp+web+server", "mcp+search+server",
        "mcp+github+server", "mcp+slack+server", "mcp+discord+server",
        "mcp+docker+server", "mcp+kubernetes+server",
    ]
    inserted = 0
    for q in queries:
        raw = fetch(f"https://api.github.com/search/repositories?q={q}&sort=stars&order=desc&per_page=100")
        if not raw:
            time.sleep(4)
            continue
        try:
        ...
        for item in data.get("items", []):
            servers.append({"name": item["full_name"], "url": item["html_url"], "description": item.get("description","") or ""})
...
def scrape_npm(db, existing):
    ...
    queries = ["mcp-server", "mcp+server", "model-context-protocol", "@modelcontextprotocol/server",
               "mcp+tool", "mcp+client", "mcp+proxy", "mcp+gateway", "mcp+aggregator", "mcp+bridge"]
    inserted = 0
    for q in queries:
        ...
        for obj in data.get("objects", []):
            ...
            is_mcp = any(k in ["mcp","mcp-server","model-context-protocol"] for k in keywords)
            if not is_mcp and "mcp" not in name.lower() and "mcp" not in desc.lower(): continue
...
def scrape_pypi(db, existing):
    ...
    for q in ["mcp-server", "mcp-server-", "model-context-protocol"]:
        ...
        if raw:
            try:
                data = json.loads(raw)
                info = data.get("info", {})
                servers = [{"name": info.get("name",""), "url": info.get("project_url","") or f"https://pypi.org/project/{info.get('name','')}/", "description": info.get("summary","")}]
    ...
    if raw:
        mcp_pkgs = re.findall(r'<a[^>]*>([^<]*(?:mcp|model.context.protocol)[^<]*)</a>', raw, re.IGNORECASE)
        servers = []
        for pkg in mcp_pkgs[:300]:
            servers.append({"name": pkg, "url": f"https://pypi.org/project/{pkg}/", "description": f"PyPI package: {pkg}"})
        inserted += insert_batch(db, servers, "pypi", existing)
    print(f"  +{inserted}")
    return inserted
...
def scrape_curated_lists(db, existing):
    ...
    urls = [
        ("https://raw.githubusercontent.com/wong2/awesome-mcp-servers/main/README.md", "wong2-awesome"),
        ("https://raw.githubusercontent.com/AI-Engineer-Foundation/awesome-mcp-servers/main/README.md", "aief-awesome"),
        ("https://raw.githubusercontent.com/michaellatman/mcp-get/main/README.md", "mcp-get"),
        ("https://raw.githubusercontent.com/taketwo/awesome-mcp-servers/main/README.md", "taketwo-awesome"),
        ("https://raw.githubusercontent.com/HollyAdequateAI/awesome-mcp-servers/main/README.md", "holly-awesome"),
    ]
    inserted = 0
    for url, source in urls:
        ...
        if not raw: continue
        servers = []
        for name, gh_url in re.findall(r'\[([^\]]+)\]\((https://github\.com/[^)]+)\)', raw):
...
def scrape_smithery(db, existing):
    ...
    for page in range(1, 6):
        ...
        for srv in data.get("servers", data.get("results", [])):
            servers.append({"name": srv.get("name",""), "url": srv.get("url","") or srv.get("homepage",""), "description": srv.get("description","")})
...
def scrape_docker_hub(db, existing):
    ...
    raw = fetch("https://hub.docker.com/v2/search/repositories/?query=mcp+server&page_size=100")
    if not raw:
        print("  FAILED")
        return 0
    try:
...
def main():
    ...
    by_source = db.execute("SELECT source, count(*) FROM links_backlog GROUP BY source ORDER BY count(*) DESC").fetchall()
    db.close()
