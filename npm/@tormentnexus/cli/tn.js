...
async function main() {
  ...
  switch (cmd) {
    case "status": {
      ...
      console.log(`   🛠️  ${d.cli?.toolCount || 0} tools | 🧠 ${d.memory?.l2?.count || 0} memories`);
      console.log(`   🔗 ${TN_URL}\n`);
      break;
    }
    case "search": {
      ...
      results.forEach((m, i) => console.log(`[${i + 1}] ${(m.content || JSON.stringify(m)).slice(0, 120)}`));
      break;
    }
    case "store": {
      const content = argv.slice(1).join(" ");
      const r = await request("memory/store", "POST", { content, tags: [], category: "cli" });
