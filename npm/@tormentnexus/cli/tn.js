...
async function main() {
  ...
  switch (cmd) {
    case "status": {
      ...
      console.log(`   🛠️  ${d.cli.toolCount} tools | 🧠 ${d.memory.l2.count} memories`);
      console.log(`   🔗 ${TN_URL}\n`);
      break;
    }
    case "search": {
      ...
      if (!results.length) return console.log(`No memories found for: "${query}"`);
      results.forEach((m, i) => console.log(`[${i + 1}] ${m.content?.slice(0, 120) || JSON.stringify(m).slice(0, 120)}`));
      break;
    }
    case "store": {
      const content = argv.slice(1).join(" ") || "";
      const r = request("memory/store", "POST", { content, tags: [], category: "cli" });
    ...
    case "install": {
      ...
      const installer = path.join(__dirname, "..", "..", "..", "scripts", SYSTEM === "win32" ? "install_codewhale.bat" : "install_codewhale.sh");
