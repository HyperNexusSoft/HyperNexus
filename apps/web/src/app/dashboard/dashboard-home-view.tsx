...
export function DashboardHomeView({
	...
	return (
		<div className="min-h-screen bg-slate-950 text-slate-100 flex">
			{/* STICKY LEFT SIDEBAR */}
			<aside
				className={`fixed inset-y-0 left-0 z-20 flex w-72 flex-col border-r border-slate-800 bg-slate-900/60 backdrop-blur-xl transition-all duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:sticky md:translate-x-0`}
			>
				<div className="flex h-16 items-center justify-between px-6 border-b border-slate-800/60">
					<span className="text-sm font-bold tracking-wider text-cyan-400 font-mono">
						NAVIGATIONAL DECK
					</span>
					<button
					...
					{[
						{
							...
							desc: "Core commands, supervisor & swarm",
						},
						{
							href: "#research-workflows",
							label: "🔬 Research & Workflows",
							desc: "Topic conducting & agent steps",
						},
						{
							href: "#memory-graphrag",
							label: "🧠 Memory & GraphRAG",
							desc: "Tiers, semantic graph & cold cache",
						},
						{
							href: "#mcp-registry",
							label: "🔌 MCP & Tool Registry",
							desc: "Smithery & always-on accessory",
						},
						{
							href: "#integrations",
							label: "☁️ Integrations Hub",
							desc: "Extension bridges & Jules",
						},
						{
							href: "#governance-billing",
							label: "💼 Governance & Billing",
							desc: "Stripe, settings, RBAC & docs",
						},
					].map((item) => (
						<a
							key={item.href}
			...
			<div className="flex-1 flex flex-col min-w-0">
				...
				<div className="md:hidden flex h-14 items-center px-4 border-b border-slate-800 bg-slate-900/60 backdrop-blur-xl">
					<button
						...
						className="text-slate-200 p-2 font-black text-sm"
					>
						☰ Navigation Menu
					</button>
				</div>
						...
						<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
							<div>
								...
								<p className="text-xs text-slate-400 mt-1">
									Multilingual semantic graph routing, autonomic self-healing,
									and memory dreaming loop control surface.
								</p>
							</div>
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 flex flex-col justify-between md:col-span-1">
								<div>
									...
									<p className="text-xs text-slate-400">
										...
										repair 2,003 missing sessions and populate 15,753 lost
										backlog link entries.
									</p>
								</div>
						...
						<div className="grid gap-6 md:grid-cols-2">
									...
									<div className="border border-slate-850 bg-zinc-950/40 p-4 rounded space-y-3 text-xs">
										...
										<div className="space-y-2.5">
											...
											<label className="flex items-start gap-2.5 cursor-pointer text-slate-300 hover:text-white transition-colors">
												...
												/>
												<span>
													Router alias profile matches pass CI testing
												</span>
											</label>
											<label className="flex items-start gap-2.5 cursor-pointer text-slate-300 hover:text-white transition-colors">
												<input
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 md:col-span-2 flex flex-col justify-between">
								<div>
									...
									<p className="text-xs text-slate-400 mt-1">
										Triggers the swarm_v7.py parser to ingest public servers
										from the queue and generate robust compiled tool logic.
									</p>
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 md:col-span-2">
								...
								<div className="border border-slate-850 bg-slate-950/80 p-4 rounded-lg text-xs space-y-2">
									...
									<div className="font-mono text-[11px] text-cyan-300 max-h-[120px] overflow-y-auto space-y-1">
										<div>
											[03:14:02] [Kernel] Swarm initialized. Active
											communication channels opened on port 3001.
										</div>
										<div>
										...
										<div>
											[03:14:07] [Kernel] System state is stable. Waiting for
											next instruction.
										</div>
									</div>
								</div>
						...
						<div className="grid gap-6 md:grid-cols-2">
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 flex flex-col justify-between md:col-span-1">
								<div>
									...
									<p className="text-xs text-slate-400 mt-1">
										Safely run migrations (ALTER TABLE language,
										mcp_server_json, env_vars_found, github_topics) to ingest
										node topologies.
									</p>
								...
								<div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
									{importedSessions.length === 0 && !importLoading && (
										<div className="text-center py-8 text-slate-500 bg-zinc-950/30 border border-slate-850 rounded-lg">
											...
											<p className="text-[10px] mt-1 text-slate-500 max-w-md mx-auto">
												Click "Scan for Sessions" to sweep project workspaces
												for external transcript formats.
											</p>
										</div>
									)}
													...
													>
														...
														{session.valid && (
															<div className="pt-2 flex flex-wrap gap-2">
																...
																<button
																	onClick={() =>
																		restoreImportedSession(session)
																	}
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 md:col-span-2 font-mono">
								...
								<p className="text-xs text-slate-400">
									Real-time system errors captured, auto-diagnosed, and
									corrected.
								</p>
							...
							<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-4 md:col-span-2 font-mono">
								...
								<div className="space-y-3">
									...
									(installArtifactsQuery.data as any[]).length > 0 ? (
										(installArtifactsQuery.data as any[]).map(
											(surface: any) => (
												<div
													key={surface.id}
										...
										<div className="space-y-3 text-xs">
											<div className="border border-slate-850 bg-zinc-955 p-3 rounded-lg flex flex-col justify-between space-y-1.5">
												...
												<p className="text-[11px] text-slate-400">
													Captures web search context logs, screenshots, and
													live CDP channels.
												</p>
												<div className="text-[10px] text-slate-500 font-mono">
