...
function getPlatform() {
  ...
  if (platform === "win32") {
    ...
    return { name: `darwin-${suffix}`, ext: ".tar.gz", binary: "tormentnexus" };
  } else if (platform === "linux") {
    const suffix = arch === "arm64" ? "arm64" : "amd64";
    return { name: `linux-${suffix}`, ext: ".tar.gz", binary: "tormentnexus" };
  }
...
async function install() {
  ...
  try {
    ...
    const packageName =
      platform.ext === ".zip"
        ? `tormentnexus-${platform.name}${platform.ext}`
        : `tormentnexus-${platform.name}${platform.ext}`;
